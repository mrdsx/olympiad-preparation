...
def test_get_word_with_anagrams_success(
    test_client: TestClient, is_valid_word: Callable[[Any], bool]
):
...
def test_get_anagrams_success(
    ...
    response = test_client.get("/word-game/anagrams?base_word=диаспора")
    data = AnagramsResponse(**response.json())
...
def test_get_anagrams_unknown_word_returns_empty_list(test_client: TestClient):
